...
- `App::RenderLoop` runs on its own thread: once per second it checks `SystemMonitor` (battery / fullscreen app) for auto-pause and refreshes the `maxFPS` budget. It renders + presents otherwise, **frame-limited to `config.maxFPS`** (VSync alone would run at monitor refresh), and stops rendering when `Present` returns `DXGI_STATUS_OCCLUDED` (fully covered), polling `TestOcclusion()` until visible again. Manual pause (`m_isPaused`) comes from the tray. `WM_DISPLAYCHANGE` resizes the window + swap chain to the new virtual-desktop size.
